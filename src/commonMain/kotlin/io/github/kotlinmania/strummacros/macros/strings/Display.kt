...
internal fun displayInner(ast: DeriveInput): SynResult<TokenStream> {
    ...
    for (variant in variants) {
        ...
        val params = when (val fields = variant.fields) {
            ...
            is Fields.Unnamed -> {
                val names = (0 until fields.fields.unnamed.len()).map { i ->
            ...
            is Fields.Named -> {
                val names = fields.fields.named.toList().map { field ->
        ...
        val arm = when (val fields = variant.fields) {
            is Fields.Named -> {
                ...
                if (usedVars.isEmpty()) {
                    ...
                    val args = fields.fields.named.toList().mapNotNull { field ->
                        val fieldIdent = field.ident!!
            ...
            is Fields.Unnamed -> {
                ...
                if (usedVars.isEmpty()) {
                    ...
                    val args = (0 until fields.fields.unnamed.len()).map { i ->
                        formatIdent("field{}", i)
                    }
                    quote(
                        """
...
private fun captureFormatStringIdents(stringLiteral: LitStr): SynResult<List<Ident>> {
    ...
    for (identStr in rawList) {
        val parsed = parseStr(IdentParse::parse, identStr).getOrElse {
            return SynResult.failure(
                SynError.new(
                    stringLiteral.span(),
...
private fun captureFormatStrings(stringLiteral: LitStr): SynResult<List<String>> {
    ...
    for (i in formatStr.indices) {
        ...
        if (chr == '}') {
            val startIndex = newVarStartIndex ?: return SynResult.failure(
                SynError.new(
                    stringLiteral.span(),
