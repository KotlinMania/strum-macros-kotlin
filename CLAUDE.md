# Claude Code Project Instructions — strum-macros-kotlin

## Project Overview

This is **strum-macros-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`strum_macros`](https://crates.io/crates/strum_macros).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/strum_macros/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/strum_macros>
